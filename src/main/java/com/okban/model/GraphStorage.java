...
public class GraphStorage {
    ...
    public void addEdge(int from, int destId, double edgeCost, boolean reverse, int[] shape) {
    ...
    public int appendShape(int shape[]) {
        if (shape == null || shape.length == 0)
            return -1;
        // ensure capacity based on used count (shapeNodeUsed) + new length
    ...
    public void ensureEdgeCapacity(int min) {
        if (min <= to.length)
            return;
        int newCap = Math.max(min, to.length * 2);
    ...
    public void compact() {
        ...
        xs = Arrays.copyOf(xs, Math.max(0, nodeCount + 10));
        ys = Arrays.copyOf(ys, Math.max(0, nodeCount + 10));
        lons = Arrays.copyOf(lons, Math.max(0, nodeCount + 10));
        lats = Arrays.copyOf(lats, Math.max(0, nodeCount + 10));
        head = Arrays.copyOf(head, Math.max(0, nodeCount + 10));

        to = Arrays.copyOf(to, Math.max(0, edgeCount + 10));
        next = Arrays.copyOf(next, Math.max(0, edgeCount + 10));
        cost = Arrays.copyOf(cost, Math.max(0, edgeCount + 10));
        isReverse = Arrays.copyOf(isReverse, Math.max(0, edgeCount + 10));
        shapeOffSets = Arrays.copyOf(shapeOffSets, Math.max(0, edgeCount + 10));
        shapeLengths = Arrays.copyOf(shapeLengths, Math.max(0, edgeCount + 10));

        shapeNodes = Arrays.copyOf(shapeNodes, Math.max(0, shapeNodeUsed + 10));
    }
